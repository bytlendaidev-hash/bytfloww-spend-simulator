...
export const SpendTrendsTab: React.FC<SpendTrendsTabProps> = ({
  ...
  return (
    <div className="space-y-4 max-w-4xl mx-auto pb-8">
                ...
                <div key={d.day} className="flex-1 flex flex-col items-center h-full justify-end group relative">
                  <div
                    ...
                    className={`w-full rounded-t-sm transition-all duration-300 ${
                      isPeak 
                        ? 'bg-brand-viridian shadow-lg shadow-brand-viridian/40' 
                        : d.spend > 0 
      ...
      <div className={`p-6 sm:p-7 rounded-[28px] border space-y-3 transition-all duration-200 ${
        ...
        <div className="pt-3">
          <svg className="w-full h-36" viewBox="0 0 400 120">
            <path d={svgPathD} fill={isDark ? 'rgba(0, 200, 150, 0.12)' : 'rgba(5, 150, 105, 0.10)'} />
            <path d={svgLineD} fill="none" stroke={isDark ? '#00C896' : '#059669'} strokeWidth="2.5" />
            <circle cx="400" cy={cumulativePoints[cumulativePoints.length - 1]?.y || 20} r="4" fill={isDark ? '#00C896' : '#059669'} stroke="#FFFFFF" strokeWidth="2" />
          </svg>
      ...
      <div className={`p-6 rounded-[28px] border space-y-4 transition-all duration-200 ${
        ...
        <div className="flex items-center gap-2">
          <span className="text-base text-selvex-500">🌀</span>
          <h3 className={`text-base font-black tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div className={`p-4 rounded-2xl border transition-all ${
            isDark ? 'bg-[#0A2420] border-brand-viridian/20' : 'bg-emerald-50/70 border-brand-200'
          }`}>
            <span className={`text-[10px] block font-bold uppercase tracking-wider ${isDark ? 'text-brand-300' : 'text-brand-700'}`}>Daily Avg Velocity</span>
            <div className={`text-base sm:text-lg font-black font-mono mt-1 ${isDark ? 'text-brand-viridian' : 'text-brand-800'}`}>
          ...
          <div className={`p-4 rounded-2xl border transition-all ${
            ...
            <div className="text-base sm:text-lg font-black font-mono text-rose-600 dark:text-rose-400 mt-1">
