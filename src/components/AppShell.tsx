...
export const AppShell: React.FC<AppShellProps> = ({
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 relative overflow-x-hidden p-2 sm:p-4 lg:p-6 flex flex-col justify-between ${
      isDark ? 'text-[#F8FAFC]' : 'text-[#0F172A]'
    }`}>
      {/* ── 1. AMBIENT SPATIAL ENVIRONMENTAL BACKGROUND ──────────────────── */}
      <SpatialBackground isDark={isDark} />

      {/* ── 2. VISIONOS FLOATING SPATIAL WINDOW CONTAINER ─────────────────── */}
      <div className={`w-full max-w-[1520px] mx-auto my-auto relative z-10 flex flex-col transition-all duration-300 ${
        isDark ? 'visionos-window' : 'visionos-window-light'
      }`}>
        
        {/* ── WINDOW TITLEBAR & CONTROL BAR ─────────────────────────────── */}
        <header className={`px-4 sm:px-8 py-3.5 sm:py-4 border-b flex items-center justify-between gap-3 ${
          isDark ? 'border-white/[0.12]' : 'border-slate-200/80'
        }`}>
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5 text-[10px]">
                <span className="text-emerald-400 font-bold">● Live Reconciled</span>
                <span className="text-slate-400 hidden sm:inline">• Deterministic Financial Engine</span>
              </div>
            </div>
          </div>
          ...
          {onSwitchModule && currentUser && (
            <div className={`hidden md:flex items-center p-1 rounded-2xl border backdrop-blur-xl ${
              isDark ? 'bg-black/30 border-white/[0.15]' : 'bg-slate-100/90 border-slate-200/90'
            }`}>
              <button
                onClick={() => onSwitchModule('BANK_STATEMENTS')}
                className={`px-4 py-1.5 rounded-xl text-xs font-black transition-all duration-150 flex items-center gap-1.5 ${
                  activeModule === 'BANK_STATEMENTS'
                    ? (isDark ? 'visionos-pill-active text-white' : 'bg-emerald-600 text-white shadow-md shadow-emerald-600/20')
                    : (isDark ? 'text-white/70 hover:text-white hover:bg-white/5' : 'text-slate-600 hover:text-slate-900')
                }`}
              >
                <span>📊</span>
              ...
              <button
                ...
                className={`px-4 py-1.5 rounded-xl text-xs font-black transition-all duration-150 flex items-center gap-1.5 ${
                  activeModule === 'SMS_INTELLIGENCE'
                    ? (isDark ? 'visionos-pill-active text-white' : 'bg-indigo-600 text-white shadow-md shadow-indigo-600/20')
                    : (isDark ? 'text-white/70 hover:text-white hover:bg-white/5' : 'text-slate-600 hover:text-slate-900')
                }`}
              >
                <span>📱</span>
          ...
          <div className="flex items-center gap-2 sm:gap-2.5">
            ...
            {currentUser && (
              <div className={`hidden lg:flex items-center gap-2 px-3 py-1.5 rounded-2xl border text-xs backdrop-blur-xl ${
                isDark ? 'bg-white/[0.08] border-white/[0.14]' : 'bg-white/80 border-slate-200/90 shadow-sm'
              }`}>
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3.5 py-2 rounded-2xl text-xs font-bold border transition-all duration-150 active:scale-95 backdrop-blur-xl ${
                isDark 
                  ? 'bg-white/[0.08] hover:bg-white/[0.15] border-white/[0.14] text-white' 
            ...
            <button
              ...
              className={`w-10 h-10 rounded-2xl flex items-center justify-center text-sm border transition-all duration-150 active:scale-95 backdrop-blur-xl ${
                isDark 
                  ? 'bg-white/[0.08] hover:bg-white/[0.15] border-white/[0.14] text-amber-300' 
                  : 'bg-white/90 hover:bg-slate-100 border-slate-200/90 text-slate-800 shadow-sm'
              }`}
            ...
            <button
              ...
              className={`md:hidden w-10 h-10 rounded-2xl flex items-center justify-center text-sm border transition ${
                isDark ? 'bg-white/[0.08] border-white/[0.14] text-white' : 'bg-white/90 border-slate-200/90 text-slate-900 shadow-sm'
              }`}
        ...
        {mobileMenuOpen && (
          <div className={`md:hidden px-4 py-4 border-b space-y-3 animate-fade-in backdrop-blur-2xl ${
            isDark ? 'bg-black/80 border-white/[0.12]' : 'bg-white/95 border-slate-200/90 shadow-xl'
          }`}>
            {currentUser && (
        ...
        <footer className={`px-6 py-4 border-t text-center text-xs transition-colors relative z-10 flex flex-col sm:flex-row items-center justify-between gap-2 ${
          isDark ? 'border-white/[0.08] text-white/50' : 'border-slate-200/80 text-slate-400'
        }`}>
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="font-semibold text-white/80">BytFloww Spatial Financial OS</span>
            <span>•</span>
            <span>visionOS Precision Glass</span>
          </div>
