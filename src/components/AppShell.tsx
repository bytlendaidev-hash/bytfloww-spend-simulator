...
export const AppShell: React.FC<AppShellProps> = ({
  ...
  const currentTheme = THEME_TEMPLATES[currentThemeId] || THEME_TEMPLATES.bytlend_vision;

  return (
    <div className="h-screen w-screen relative overflow-hidden flex flex-col bg-abyss-canvas text-abyss-textPrimary selection:bg-emerald-500/30 selection:text-white transition-colors duration-300">
      ...
      <header className="h-14 sm:h-16 px-4 sm:px-6 border-b border-abyss-border flex items-center justify-between gap-3 shrink-0 z-40 bg-abyss-card/90 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <div className="flex items-center gap-2">
            <span className="font-extrabold text-base sm:text-lg tracking-tight text-abyss-textPrimary">
              Byt<span className="text-[#00884E] dark:text-[#1AE893]">Floww</span>
            </span>
            <span className="hidden sm:inline-flex text-[9px] font-mono font-bold px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-700 dark:text-[#1AE893] border border-emerald-500/20 tracking-wider uppercase items-center gap-1">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="spatial-btn px-3 py-1.5 text-xs flex items-center gap-1.5 border-amber-500/30 text-amber-600 dark:text-[#E0A83F] hover:border-amber-400"
            title="Theme Studio"
          >
            <span>🎨</span>
          ...
          {currentUser && (
            <div className="flex items-center gap-2 px-2.5 sm:px-3 py-1 rounded-full bg-abyss-well border border-abyss-border text-xs">
              <div className="w-6 h-6 rounded-full bg-emerald-500/15 border border-emerald-500/30 flex items-center justify-center text-[10px] font-bold text-emerald-700 dark:text-[#1AE893]">
