...
export const SpendOverviewTab: React.FC<SpendOverviewTabProps> = ({
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-12 animate-emergence">
      ...
      <div className="spatial-card-hero p-6 sm:p-8 space-y-6 relative overflow-hidden">
        ...
        <div className="flex items-center justify-between gap-3 relative z-10 flex-wrap">
          ...
          <span className="px-3.5 py-1 rounded-full text-[11px] font-bold tracking-wider border bg-synapse-500/20 text-synapse-400 border-synapse-500/40 shadow-sm flex items-center gap-1.5">
            <span>✨</span> {snapshot.healthScoreTier} CAPITAL HEALTH TIER
          </span>
        </div>
        ...
        <div className="relative z-10 pt-2">
          ...
          <div className="text-5xl sm:text-7xl font-black tracking-tight font-mono mt-1 flex items-baseline gap-1 text-pulse-500">
            <span className="text-3xl sm:text-5xl font-normal opacity-80">₹</span>
            <span>
        ...
        <div className="space-y-3 pt-2 relative z-10 border-t border-pulse-500/20">
          ...
          <div className="h-3 rounded-full bg-abyss-well border border-abyss-border overflow-hidden p-0.5 shadow-inner">
            <div 
      ...
      <div className="spatial-card p-5 sm:p-6 grid grid-cols-3 gap-2 sm:gap-4 text-left border-jade-500/25 shadow-lg">
        ...
        <div className="space-y-1">
          ...
          <div className="text-sm sm:text-2xl font-bold font-mono text-jade-500 truncate">
        ...
        <div className="space-y-1 border-x border-abyss-border px-2 sm:px-4">
          ...
          <div className="text-sm sm:text-2xl font-bold font-mono text-pulse-500 truncate">
            ₹{snapshot.totalSpend.toLocaleString('en-IN')}
          </div>
          <div className="text-[10px] text-abyss-textMuted hidden sm:block">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div 
          ...
          className="spatial-card p-5 flex items-center justify-between cursor-pointer group hover:border-ochre-500/50"
        >
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-[16px] bg-ochre-500/15 border border-ochre-500/30 flex items-center justify-center text-2xl shrink-0 group-hover:scale-105 transition-transform">
              ☀️
            </div>
            <div>
          ...
          <span className="text-lg font-bold text-abyss-textMuted group-hover:text-ochre-500 transition">→</span>
        </div>

        {/* Evening Reflection */}
        <div 
          onClick={() => onOpenDebrief()}
          className="spatial-card p-5 flex items-center justify-between cursor-pointer group hover:border-synapse-500/50"
        >
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-[16px] bg-synapse-500/15 border border-synapse-500/30 flex items-center justify-center text-2xl shrink-0 group-hover:scale-105 transition-transform">
              🌙
            </div>
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-synapse-500/20 text-synapse-400 border border-synapse-500/30">
                  10 PM
                </span>
              </div>
              <p className="text-xs text-abyss-textMuted mt-0.5 font-medium">
          ...
          <span className="text-lg font-bold text-abyss-textMuted group-hover:text-synapse-500 transition">→</span>
        </div>
      </div>
      ...
      <div className="spatial-card p-6 sm:p-7 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-abyss-textMuted font-medium">
              Deterministic category distribution • {snapshot.categoryDistribution.length} distinct sectors
            </p>
          </div>
          <button 
            onClick={() => onNavigateToTab('CATEGORIES')}
            className="spatial-btn px-4 py-1.5 text-xs text-jade-500 border-jade-500/30"
          >
        ...
        <div className="h-3.5 rounded-full overflow-hidden flex gap-1 p-0.5 bg-abyss-well border border-abyss-border shadow-inner">
          ...
          {snapshot.categoryDistribution.slice(0, 5).map((cat) => {
            ...
            return (
              <div 
                key={cat.category}
                onClick={() => onSelectCategory ? onSelectCategory(cat) : onNavigateToTab('CATEGORIES')}
                className="p-3.5 rounded-[14px] bg-abyss-well border border-abyss-border hover:border-abyss-borderStrong hover:bg-abyss-elevated cursor-pointer transition-all duration-200 space-y-2"
              >
                <div className="flex items-center justify-between text-xs sm:text-sm">
                  <div className="flex items-center gap-2.5">
                    <span className="w-3 h-3 rounded-full shadow-sm shrink-0" style={{ backgroundColor: catColors.solid }} />
                    <span className="font-semibold text-abyss-textPrimary tracking-tight truncate max-w-[160px] sm:max-w-xs">
                      {cat.category}
                    </span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="font-mono text-xs text-abyss-textMuted font-bold px-2 py-0.5 rounded-full bg-abyss-card border border-abyss-border">
                      {cat.pct}%
                    </span>
                ...
                {/* Progress Track */}
                <div className="h-1.5 rounded-full bg-abyss-card overflow-hidden">
                  <div 
      ...
      <div className="spatial-card p-6 sm:p-7 space-y-3.5 border-l-4 border-l-synapse-500 shadow-lg">
        <div className="flex items-center justify-between">
          ...
          <button 
            ...
            className="spatial-btn px-4 py-1.5 text-xs text-synapse-400 border-synapse-500/40 bg-synapse-500/10"
          >
            Ask Copilot →
          </button>
        </div>
        ...
        <div className="space-y-2.5 text-xs sm:text-sm">
          ...
          <div className="flex items-start gap-2 pt-2 text-xs text-abyss-textMuted border-t border-abyss-border">
            <span>💡</span>
            <span>Local ledger analysis processed {snapshot.transactionCount} transactions with 100% deterministic privacy & zero cloud leakage.</span>
          </div>
        </div>
      </div>
      ...
      <div className="spatial-card p-6 sm:p-7 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button 
            ...
            className="spatial-btn px-4 py-1.5 text-xs text-ochre-500 border-ochre-500/30"
          >
        ...
        >
          <div className="p-4 rounded-[14px] bg-abyss-well border border-abyss-border hover:bg-abyss-elevated transition">
            <span className="text-xs text-ochre-500 font-bold block">EMIs & Loans</span>
          ...
          <div className="p-4 rounded-[14px] bg-abyss-well border border-abyss-border hover:bg-abyss-elevated transition">
            <span className="text-xs text-pulse-500 font-bold block">Subscriptions</span>
          ...
          <div className="p-4 rounded-[14px] bg-abyss-well border border-abyss-border hover:bg-abyss-elevated transition">
            <span className="text-xs text-telemetry-500 font-bold block">Utility Bills</span>
          ...
          {snapshot.accounts.map((acc) => {
            ...
            return (
              <div 
                ...
                className="spatial-card p-6 space-y-4 cursor-pointer group hover:border-jade-500/40"
              >
        ...
        <div className="grid grid-cols-1 gap-4">
          {snapshot.creditCards.slice(0, 2).map((card) => (
            <div 
              ...
              className="spatial-card p-6 relative overflow-hidden cursor-pointer group hover:border-synapse-500/40"
            >
              ...
              <div className="grid grid-cols-2 gap-4 my-4 relative z-10">
                <div>
                  <span className="text-[10px] uppercase tracking-wider text-abyss-textMuted block">
                    Available Limit
                  </span>
                  <span className="text-xl sm:text-2xl font-bold font-mono text-jade-500">
                ...
                <div className="text-right">
                  ...
                  <span className="text-xl sm:text-2xl font-bold font-mono text-abyss-textPrimary">
              ...
              <div className="flex items-center justify-between text-xs text-abyss-textMuted pt-3 relative z-10">
                <span>Total debits this cycle: <strong className="text-pulse-500 font-mono">₹{card.totalDebits.toLocaleString('en-IN')}</strong></span>
