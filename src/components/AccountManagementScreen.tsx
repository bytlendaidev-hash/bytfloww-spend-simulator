  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-8">
      ...
      <div className="space-y-4">
        <h4 className={`text-sm font-black uppercase tracking-wider px-1 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {accounts.map((acc) => (
            <div
              ...
              className={`p-6 rounded-[28px] border transition-all duration-150 cursor-pointer hover:scale-[1.01] active:scale-[0.99] group ${
                isDark 
                  ? 'bg-[#10181E] border-white/[0.06] hover:border-brand-viridian/40 shadow-xl shadow-black/40' 
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className={`text-sm sm:text-base font-black tracking-tight truncate ${isDark ? 'text-white' : 'text-slate-900'}`}>{acc.institution}</div>
                ...
                <div className="text-right flex-shrink-0 pl-2">
                  <div className={`text-base sm:text-lg font-black font-mono ${isDark ? 'text-brand-viridian' : 'text-brand-700'}`}>
              ...
              <div className={`grid grid-cols-3 gap-2 pt-3 border-t text-xs text-center ${
                ...
                <div className={`p-2.5 rounded-xl ${isDark ? 'bg-[#142027]' : 'bg-slate-50'}`}>
                  <span className={`block text-[10px] font-bold ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Total Debits</span>
                  <span className="font-black text-rose-600 dark:text-rose-400 font-mono">₹{Math.round(acc.totalDebits).toLocaleString('en-IN')}</span>
                </div>
                <div className={`p-2.5 rounded-xl ${isDark ? 'bg-[#142027]' : 'bg-slate-50'}`}>
                  <span className={`block text-[10px] font-bold ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Total Credits</span>
                  <span className={`font-black font-mono ${isDark ? 'text-brand-viridian' : 'text-brand-700'}`}>₹{Math.round(acc.totalCredits).toLocaleString('en-IN')}</span>
                </div>
                <div className={`p-2.5 rounded-xl ${isDark ? 'bg-[#142027]' : 'bg-slate-50'}`}>
