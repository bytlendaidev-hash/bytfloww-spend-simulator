...
export const BytLendLogo: React.FC<BytLendLogoProps> = ({
  ...
  return (
    <div className={`flex items-center gap-3.5 ${className}`}>
      {/* Precision Squircle Emblem with Specular Glow Border */}
      <div 
        className={`${sizeMap[size]} relative flex items-center justify-center p-2 shrink-0 bg-abyss-card border border-jade-500/40 shadow-lg shadow-jade-500/15 transition-transform duration-200 hover:scale-105 group overflow-hidden`}
      >
        {/* Ambient Radial Glow inside logo */}
        <div className="absolute inset-0 bg-jade-500/15 rounded-[inherit] pointer-events-none" />
        ...
        >
          {/* 1. Left Vertical Spine Pill (Bar 1 - Cyber Emerald) */}
          <path
            ...
            style={{ fill: 'var(--color-jade-500, #00F5A0)' }}
          />

          {/* 2. Top-Right Angled Diagonal Pill (Bar 2 - Hyper Violet) */}
          <path
            ...
            style={{ fill: 'var(--color-synapse-500, #8B5CF6)' }}
          />

          {/* 3. Middle Arrowhead Chevron (Bar 3 - Cyber Emerald) */}
          <path
          ...
          {/* 4. Bottom-Right Angled Diagonal Leg (Bar 4 - Hyper Violet) */}
          <path
      ...
      {/* Billion-Dollar Brand Typography */}
      {showText && (
        <div>
          <div className="flex items-center text-xl sm:text-2xl font-bold tracking-tight">
            <span className="text-abyss-textPrimary font-bold">Byt</span>
            <span className="text-jade-500 ml-0.5 font-black">
              Floww
            </span>
          </div>
          <div className="text-[9px] font-bold tracking-[0.25em] text-synapse-500 uppercase mt-0.5">
